...
/*
    @Autowired
    private CananoUserDetailsServiceImpl cananoUserDetailsService;
*/
    @Autowired
    ...
    public void saveCollaborationGroup(CollaborationGroupBean collaborationGroup) throws CommunityException, NoAccessException, DuplicateEntriesException
    {
        ...
        try {
            ...
            // update existing group
				...
				// if (!ud.getGroups().contains(collaborationGroup.getOriginalName()))  //  JIRA CANANOLAB-601 **************************
                if (!userDao.getUserGroups( userDetails.getUsername() ).contains(collaborationGroup.getOriginalName())){
...
*/
        ...
        groupService.updateGroupWithName( groupId, collaborationGroup.getDescription(), collaborationGroup.getName() );
        ...
        try {
            if (groupList != null && groupList.size() > 0)
            {
                for (Group group : groupList)
                {
                    ...
                    cGroup.setOriginalName(group.getGroupName());
